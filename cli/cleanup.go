...
func cleanUp(*cobra.Command, []string) {
	configFile := ".gke.yml"
	...
	directories := viper.GetStringSlice("cleanupsequence")
	...
	for _, dir := range directories {
		fApply(dir, clusterName+"-"+dir, logFile)
		fDestroy(dir, clusterName+"-"+dir, logFile)
	}

}
func fApply(workingDir string, workspace string, logFile string) {
	...
	v := tfexec.Var("force_destroy=true")
	err = tf.Apply(context.Background(), c, v)
...
func fDestroy(workingDir string, workspace string, logFile string) {
	execPath := "/usr/local/bin/terraform"
	...
	c := tfexec.VarFile("../terraform.tfvars")
	v := tfexec.Var("force_destroy=true")
	err = tf.Destroy(context.Background(), c, v)
